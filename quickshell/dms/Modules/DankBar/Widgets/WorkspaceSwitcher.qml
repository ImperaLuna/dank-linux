...
Item {
    ...
    property int specialClientMonitorId: -1
    property int specialOpenMonitorId: -1
    readonly property var sortedToplevels: {
        return CompositorService.filterCurrentWorkspace(CompositorService.sortedToplevels, screenName);
    }
    readonly property var hyprSpecialWorkspaceSource: {
        if (!CompositorService.isHyprland)
            return null;
        const workspaces = Hyprland.workspaces?.values || [];
        return workspaces.find(ws => (ws?.name ?? "").startsWith("special:")) || null;
    }
    readonly property int hyprSpecialWorkspaceId: hyprSpecialWorkspaceSource?.id ?? -98
    readonly property string hyprSpecialWorkspaceRawName: hyprSpecialWorkspaceSource?.name ?? "special:magic"
    readonly property string hyprSpecialWorkspaceToggleName: {
        const raw = root.hyprSpecialWorkspaceRawName;
        return raw.startsWith("special:") ? raw.substring(8) : "magic";
    }
    readonly property string hyprOpenSpecialMonitorName: {
        if (!CompositorService.isHyprland)
            return "";
        const monitors = Hyprland.monitors?.values || [];
        const mon = monitors.find(m => m?.id === root.specialOpenMonitorId);
        return mon?.name || "";
    }
    readonly property string hyprSpecialWorkspaceMonitorName: {
        if (!CompositorService.isHyprland)
            return "";
        const monitors = Hyprland.monitors?.values || [];
        const mon = monitors.find(m => m?.id === root.specialClientMonitorId);
        return mon?.name || "";
    }
    readonly property bool hyprSpecialActive: CompositorService.isHyprland && root.hyprOpenSpecialMonitorName !== ""
    readonly property bool showHyprSpecialWorkspace: CompositorService.isHyprland && (hyprSpecialActive || specialClientCount > 0)
    readonly property var hyprSpecialWorkspace: ({
            id: root.hyprSpecialWorkspaceId,
            name: "~",
            urgent: false,
            monitor: {
                name: hyprSpecialWorkspaceSource?.monitor?.name || root.effectiveScreenName
            }
        })
    ...
    Process {
        id: hyprSpecialClientProbe
        running: false
        command: [
            "sh", "-lc",
            "python3 -c 'import json,subprocess; clients=json.loads(subprocess.check_output([\"hyprctl\",\"clients\",\"-j\"]).decode()); mons=json.loads(subprocess.check_output([\"hyprctl\",\"monitors\",\"-j\"]).decode()); special=[c for c in clients if str((c.get(\"workspace\") or {}).get(\"name\",\"\")).startswith(\"special:\")]; client_monitor=-1 if not special else int(special[0].get(\"monitor\",-1)); open_monitor=-1\nfor m in mons:\n s=m.get(\"specialWorkspace\") or {}\n name=str(s.get(\"name\", \"\"))\n sid=int(s.get(\"id\", 0) or 0)\n if name.startswith(\"special:\") or sid < 0:\n  open_monitor=int(m.get(\"id\", -1))\n  break\nprint(f\"{len(special)} {client_monitor} {open_monitor}\")'"
        ]
        ...
        stdout: SplitParser {
            onRead: function(line) {
                ...
                const monitorId = parseInt(parts[1] || "");
                const openMonitorId = parseInt(parts[2] || "");
                if (!Number.isNaN(count))
                    root.specialClientCount = Math.max(0, count);
                root.specialClientMonitorId = Number.isNaN(monitorId) ? -1 : monitorId;
                root.specialOpenMonitorId = Number.isNaN(openMonitorId) ? -1 : openMonitorId;
            }
        }
    }

    Timer {
        id: hyprSpecialClientTimer
        interval: 900
        repeat: true
        running: CompositorService.isHyprland
        triggeredOnStart: true
        onTriggered: {
            if (!hyprSpecialClientProbe.running)
                hyprSpecialClientProbe.running = true;
        }
    }
    ...
    function getHyprlandWorkspaces() {
        ...
        let filtered = workspaces.filter(ws => ws && (ws.id > -1));
        ...
        if (!SettingsData.showOccupiedWorkspacesOnly) {
            if (root.showHyprSpecialWorkspace && !filtered.some(ws => ws.id === root.hyprSpecialWorkspaceId))
                filtered.push(root.hyprSpecialWorkspace);
            return filtered;
        }
    ...
    function getWorkspaceIndex(modelData, index) {
        ...
        let workspaceName = "";
        if (CompositorService.isHyprland && (modelData?.name ?? "") === "~") {
            workspaceName = "";
        } else if (SettingsData.showWorkspaceName) {
        ...
        Repeater {
            ...
            Item {
                ...
                property bool isActive: {
                    if (CompositorService.isHyprland && (modelData?.name ?? "") === "~")
                        return root.hyprOpenSpecialMonitorName !== "" && (root.hyprOpenSpecialMonitorName === root.effectiveScreenName || root.hyprOpenSpecialMonitorName === root.screenName);
                    if (root.useExtWorkspace)
                ...
                property bool isOccupied: {
                    if (CompositorService.isHyprland)
                        if ((modelData?.name ?? "") === "~")
                            return root.specialClientCount > 0;
                    ...
                    onReleased: mouse => {
                        ...
                        if (mouse.button === Qt.LeftButton) {
                            if (root.useExtWorkspace && (modelData?.id || modelData?.name)) {
                                ...
                                if ((modelData?.name ?? "") === "~") {
                                    Hyprland.dispatch(`togglespecialworkspace ${root.hyprSpecialWorkspaceToggleName}`);
                                } else {
